...
def PlayGame(femaleAgent, maleAgent):
    ...
    while not done:
        ...
        mpt, currentMoves = Run("PE", femaleAgent, maleAgent,moves=7500)
...
def finalQtable(qtable, world):
    ...
    for index, row in qtable.iterrows():
        drawBlock(int(index / 5), index % 5, shadeCell(row["N"]/ 4), row["N"], "N")
...
def plot(run1, run2):
    run1 = sns.regplot(y=run1, x=np.arange(0, len(run1), 1), color="g", label="First Run")
    ax2 = plt.twinx()
    run2 = sns.regplot(y=run2, x=np.arange(0, len(run2), 1), ax=ax2, label="Second Run")
    run1.set(ylim=(0, 800))
    run2.set(ylim=(0, 800))
...
time.sleep(0)
print("Male Final Qtable Run 1 (sep=True, x = 0):")
